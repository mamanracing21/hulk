...
def hasil_fb(ENG):
	if "bahasa_inggris" in ENG:
		tulis(Panel(f"{Te}{U} •{P} 01 {O}Check account results {H}OK \n{U} •{P} 02 {O}Check account results {K}CP \n{U} •{P} 03 {O}Remove crack results \n{U} •{M} 00 {O}Return",style='#FF0022'))
		rom = input('%s╰─%s Select %s>%s '%(p,o,m,k))
		if rom in['']:
			...
			if len(oke)==0:
				...
				tulis(Panel(f"{Te}{U} •{O} Account results{H} OK{O} stored in your folder ",style='#FF0022'))
				nomor = 0
				for i in oke:
				...
				tulis(Panel(f"{Te}{U} •{O} result date{M} : {H}{file_nm} {O}amount {M}: {H}{len(totalok)}",style='#FF0022'))
			...
			if len(cepe)==0:
				...
				tulis(Panel(f"{Te}{U} •{O} result date{M} : {K}{file_nm} {O}amount {M}: {K}{len(totalcp)}",style='#FF0022'))
			...
			ld().loads()
			jalan (f'{m} successfully removed the crack result ');jeda(2)
			os.system("python bff-2.py")
		elif rom in['0','00']:
			os.system("python bff-2.py")
		else:
			print ('\n%s╰─%s choose the right one'%(p,m));jeda(2)
			os.system("python bff-2.py")
